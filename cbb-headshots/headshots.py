...
# TODO: implement all
LOCATOR: Callable[[Page | Locator], Locator] = lambda locator: locator.locator("select#sidearm-roster-select-template-dropdown")
POPUP_LOCATOR: Callable[[Page | Locator], Locator] = lambda locator: locator.locator("#iubenda-cs-banner")
PLAYERS_LOCATOR: Callable[[Page | Locator], Locator] = lambda locator: locator.locator("li.sidearm-list-card-item[data-player-id]")
PLAYERS_JERSEY_LOCATOR: Callable[[Page | Locator], Locator] = lambda locator: locator.locator(".sidearm-roster-player-image .sidearm-roster-player-jersey")
PLAYERS_HEADSHOT_LOCATOR: Callable[[Page | Locator], Locator] = lambda locator: locator.locator(".sidearm-roster-player-image")
...
def get_img_url(locator: Locator) -> str | None:
    ...
    img: dict[str, str] = locator.evaluate("el => el.src")
    ...
    query = {
        ...
        if all(q not in k for q in ("width", "height"))
    }
    if "url" in query:
...
def get_headshots(context: BrowserContext, url: str) -> Generator[dict[str, str | None], None, None]:
    def _get_headshot(locator: Page | Locator) -> dict[str, str | None]:
        jersey = PLAYERS_JERSEY_LOCATOR(locator)
        headshot = PLAYERS_HEADSHOT_LOCATOR(locator)
        return {"jersey": get_number(jersey), "headshot": get_img_url(headshot)}
    ...
    with context.new_page() as page:
        ...
        match locator.first.evaluate("el => el.tagName"):
            case "SELECT":
                locator.hover()
                locator.select_option("Roster View - Cards")
            ...
            case "A" if locator.count() > 1:
                for a in locator.all():
                    with context.new_page() as temp_page:
                        temp_page.goto(a.evaluate("el => el.href"))
                        yield _get_headshot(temp_page)

# FIXME: tulsa: libpng warning: iCCP: known incorrect sRGB profile
def circular_crop_faces(detector: vision.FaceDetector, img: cv2.typing.MatLike, top_offset: float = TOP_OFFSET, bottom_offset: float = BOTTOM_OFFSET) -> Generator[cv2.typing.MatLike, None, None]:
...
def main() -> None:
    ...
    with (
        ...
        headshots = [*get_headshots(context, genius[args.genius]["URL"])]
